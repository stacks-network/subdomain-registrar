...
export class RegistrarQueueDB {
  ...
  makeTransferUpdates(transferedSubdomains: QueueRecord[], txHash: string): Promise<void> {
    const dbCmd =
      'INSERT INTO subdomain_queue ' +
  ...
  logTransferRequestorData(subdomainName: string, ownerAddress: string, ipAddress: string) {
  ...
  async updateStatusFor(
    ...
    await Promise.all(subdomains.map(name => dbRun(this.db, cmd, [status, statusMore, name])));
    return statusMore;
  }
  ...
  getOwnerAddressCount(ownerAddress: string) {
    ...
    return dbAll(this.db, lookup, [ownerAddress]).then(results => results.length);
  }

  getIPAddressCount(ipAddress: string) {
    const lookup = 'SELECT * FROM ip_info WHERE ip_address = ?';
    return dbAll(this.db, lookup, [ipAddress]).then(results => results.length);
  }
  ...
  async updateTransactionHeights(
    ...
    await Promise.all(
      transactions.map(entry => dbRun(this.db, cmd, [entry.txHash, entry.blockHeight]))
    );
  }
  ...
  async flushTrackedTransactions(
    ...
    await Promise.all(transactions.map(entry => dbRun(this.db, cmd, [entry.txHash])));
