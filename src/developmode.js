...
export function initializeBlockstackCore(
  ...
  const priorVersionRunning = pExec('docker ps | grep -q test-bsk-core')
    .then(() => {
      ...
      if (forceRestart) {
        ...
        return pExec(
          'docker stop test-bsk-core ; docker rm test-bsk-core ;' +
            ' rm -rf /tmp/.blockstack_int_test'
        )
          .then(() => false)
          .catch(err => {
            logger.warn(
              'Couldnt stop a previous running test-bsk-core -- might not have been running.'
            );
            logger.warn(err);
            return false;
          });
  ...
  return priorVersionRunning.then(running => {
    if (running) {
      return true;
    } else {
      return pExec(
        'docker run --name test-bsk-core -dt -p 16268:16268 -p 18332:18332 ' +
