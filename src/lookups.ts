...
export async function isSubdomainRegistered(fullyQualifiedAddress: String) {
  try {
    const nameInfoUrl =
      // coreApiUrl does not exist in the blockstack types, but to not change the code we silence this type issue.
      // @ts-ignore
      bskConfig.network.coreApiUrl + "/v1/names/" + fullyQualifiedAddress;
...
export function checkProofs(owner: string, zonefile: any) {
  return (
    resolveZoneFileToProfile(zonefile, owner)
      // validateProofs expects a 3rd argument (cheerio instance), but to not change the code we don't use it.
      // @ts-ignore
