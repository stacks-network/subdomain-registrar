...
export function makeHTTPServer(config) {
  ...
  app.post('/transfer', (req, res) => {
    ...
    for (let i = 0; i < subdomainsList.length; i++) {
      if (
        !subdomainsList[i].subdomainName ||
        !subdomainsList[i].owner ||
        !subdomainsList[i].signature
      ) {
        res.writeHead(400, HEADERS);
        res.write(
          JSON.stringify({
    ...
    server
      ...
      .catch(err => {
        logger.error(err);
        const message = 'Failed to validate your transfer request: ' + err.message;
        let code = 400;
  ...
  app.post('/issue_batch/', (req, res) => {
    ...
    if (!authHeader || authHeader !== `bearer ${config.adminPassword}`) {
      ...
      server.submitBatch().catch(() => logger.error('Failed to broadcast batch.'));
      res.writeHead(202, HEADERS);
      res.write(
        JSON.stringify({
          status: true,
  ...
  app.post('/check_zonefiles/', (req, res) => {
    ...
    if (!authHeader || authHeader !== `bearer ${config.adminPassword}`) {
      ...
      server.checkZonefiles().catch(() => logger.error('Failed to check our zonefiles.'));
      res.writeHead(202, HEADERS);
      res.write(
        JSON.stringify({
          status: true,
  ...
  const zonefileDelay = Math.min(2147483647, Math.floor(60000 * config.checkTransactionPeriod));
  const batchDelay = Math.min(2147483647, Math.floor(60000 * config.batchDelayPeriod));
