...
export function testSubdomainServer() {
  test('queueRegistration', t => {
    ...
    s.initializeServer()
      .then(() =>
        s
          .queueRegistration('foo', testAddress, 0, 'hello-world', 'foo')
          .then(() => t.ok(false, 'foo.foo.id should not be a valid id to queue'))
          .catch(err => {
            t.equal(err.message, 'Requested subdomain operation is invalid.');
          })
      )
      .then(() =>
        s
          .queueRegistration('bar', 'm123', 0, 'hello-world', 'bar')
      ...
      .then(() =>
        s
          .queueRegistration('bar', testAddress, 0, 'hello-world', 'foo')
      ...
      .then(() =>
        s
          .queueRegistration('ba', testAddress2, 0, 'hello-world', 'foo')
      ...
      .then(() =>
        s
          .queueRegistration('car', testAddress, 0, 'hello-world', 'foo')
          .then(() => t.ok(false, 'should not queue with a reused owner address'))
          .catch(err =>
            t.ok(err.message.startsWith('Owner', 'should not queue with same owner address'))
          )
      )
      .then(() =>
        s
          .queueRegistration('car', testAddress2, 0, 'hello-world', 'foo')
      ...
      .then(() =>
        s
          .spamCheck('car', testAddress2, 'hello-world', 'foo', 'bearer abcdefghijk')
          .then(res => t.notOk(res, 'should pass spam check when using authorization bearer token'))
      )
      .then(() =>
        s
          .getSubdomainStatus('bar')
          .then(x =>
            t.ok(
              x.status.startsWith('Subdomain is queued for update'),
              'bar.bar.id should be queued'
            )
          )
      )
      .then(() => s.updateQueueStatus(['bar'], 'txhash'))
      .then(() =>
        s
          .getSubdomainStatus('bar')
          .then(x =>
            t.ok(
              x.status.startsWith('Your subdomain was registered in transaction'),
      ...
      .then(() => s.getSubdomainInfo('tar.bar.id').then(resp => t.equal(resp.statusCode, 404)))
      .then(() => s.getSubdomainInfo('ba.bar.id').then(resp => t.equal(resp.statusCode, 400)))
      .then(() => {
      ...
      .then(() =>
        s.listSubdomainRecords(0).then(response => {
      ...
      .then(() =>
        s
          .queueRegistration(
            'ipwhitelisted1',
            'ST15RR51GKMB5A52GP5XRT1KT9B42M7NF2VHYS96F',
            0,
            'hello-world-zonefile',
            'whitelisted-ip-addr'
          )
      ...
      .then(() =>
        s
          .queueRegistration(
            'ipwhitelisted2',
            'ST6HQACZN5HN9RNK57PK4MH84GT3EV9FW5A6MA6G',
            0,
            'hello-world-zonefile',
            'whitelisted-ip-addr'
          )
  ...
  test('apiKeyOnly', t => {
    ...
    s.initializeServer()
      ...
      .then(() =>
        s
          .spamCheck('bar', testAddress2, 'hello-world', 'foo', 'bearer abcdefghijk')
          .then(res => t.notOk(res, 'should pass spam check when using authorization bearer token'))
      );
  });
  ...
  test('submitBatch', async t => {
    ...
    await s.submitBatch().catch(err => {
      const expected_error =
        'Error post transaction: {"error":"transaction rejected","reason":"not enough funds"}';
    ...
    t.ok(
      x.status.startsWith('Subdomain is queued'),
    ...
    t.equal(
      (await s.db.getTrackedTransactions()).length,
      1,
      'Should still be tracking 1 transaction'
    );
