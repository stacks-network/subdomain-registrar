...
export function unitTestOperations() {
  test('destructing a zonefile', t => {
    ...
    const destructs = zfs.map(x => destructZonefile(x));
  ...
  test('submitUpdate', t => {
    ...
    nock('https://stacks-node-api.mainnet.stacks.co').get('/v1/names/bar.id').reply(200, {
      address: testAddress,
    });
    ...
    nock('https://stacks-node-api.mainnet.stacks.co').get('/v2/fees/transfer').reply(200, 1);
    ...
    submitUpdate('bar.id', 'hello world', testSK, testSK2).then(x => t.equal(x, 'tx-hash'));
  });
  ...
  test('makeUpdateZonefile', t => {
    ...
    const updatesArray1 = [Object.assign({}, subdomainOp, { subdomainName: `${subdomainName}-0` })];
    const updatesArray2 = [];
    for (let i = 0; i < 20; i++) {
      updatesArray2.push(
        // @ts-ignore
    ...
    const update1 = makeUpdateZonefile('bar.id', uriEntry, updatesArray1, maxZonefileBytes);
    const update2 = makeUpdateZonefile('bar.id', uriEntry, updatesArray2, maxZonefileBytes);
    ...
    t.ok(
      Buffer.from(update2.zonefile, 'ascii').length < 1000,
    ...
    t.deepEqual(
      parsed1.txt.map(rec => rec.name),
      ['foo-0'],
      'outputted txt records should match expected subdomains'
    );
    t.deepEqual(
      parsed2.txt.map(rec => rec.name),
      [0, 1, 2, 3, 4, 5, 6, 7].map(x => `${subdomainName}-${x}`),
      'outputted txt records should match expected subdomains'
    );
  });
